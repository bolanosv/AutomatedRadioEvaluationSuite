function peakValues = measure_RFpeaks(input_Freq,output_PdBm,Gain,DE,PAE)
    ...
    saturation_PdBm = zeros(size(input_Freq, 1), 1);
    peak_1dB = zeros(size(input_Freq, 1), 1);
    peak_3dB = zeros(size(input_Freq, 1), 1);

    for i = 1:size(input_Freq, 2)
        % Find the saturation point
        saturation_idx = find(Gain(i, :) <= (peak_Gain(i) - 1), 1, 'first');
        if ~isempty(saturation_idx)
            saturation_PdBm(i) = output_PdBm(i, saturation_idx);
        else
            saturation_PdBm(i) = NaN;
        end
